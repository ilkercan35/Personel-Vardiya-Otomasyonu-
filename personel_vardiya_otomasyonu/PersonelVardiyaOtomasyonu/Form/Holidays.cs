...
namespace PersonelVardiyaOtomasyonu
{
	public partial class Holidays : Form
	{
        ...
        private void dgv_holidays_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0)
            {
                object nameCellValue = dgv_holidays.Rows[e.RowIndex].Cells[1].Value;
                object dateCellValue = dgv_holidays.Rows[e.RowIndex].Cells[2].Value;

                if (nameCellValue != null && dateCellValue != null)
                {
                    txt_name.Text = nameCellValue.ToString();
                    txt_date.Text = dateCellValue.ToString();
                }
                else
                {
                    // İlgili hücrelerde değer bulunmamaktaysa yapılacak işlem veya mesajı buraya ekleyebilirsiniz.
